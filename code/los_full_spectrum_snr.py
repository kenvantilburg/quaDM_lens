"""Line-of-sight (mean cosmological density) variance SNRs of the PREDICTED spectra,
through the corrected C_tilde_1 kernel: pure-CDM one-halo band (no cutoff), the
WIMP total (cutoff halos + prompt cusps), and the cusp component alone.
Galaxy lens B1422+231 (image B tangential, EPIC 0.1 muas) and cluster lens
SDSS J1029+2623 (image C tangential, 1 muas); stochastic (lowest-mode) and
acceleration channels. Same conventions as matter_power.ipynb."""
import numpy as np
import scipy as sp
...
k_fs = 1.06e6 * Mpc**-1
M_fs = (4*np.pi/3) * rho_m0 * (np.pi/k_fs)**3
...
def dn_dlnM(M, cutoff):
    ...
    return (rho_m0/M)*f_nu*abs(dlnsig(M, cutoff))
...
def conc(M, kind, cutoff):
    base = c_Einasto(M, h=cosmo.h, M_fs=(M_fs if cutoff else 1e-20*M_Solar)) if kind == 'Wang' \
        else (np.exp(-0.529*(M_fs/M)**(1/3)) if cutoff else 1.0)*_scp14_pol(M)
...
def make_D2_PS(kind, cutoff):
    ...
    return D2*_kk**3/(2*np.pi**2)/rho_m0**2
...
def Delta2_cusp(k):
    ...
    lnr = np.log(r_cusp_med) + np.linspace(-1.5, 1.5, 21)*0.6
...
# ------------------------------------------------------------------ LOS kernel
kc_grid = np.logspace(2, np.log10(3e11), 600) * Mpc**-1
...
vec_omega = 2*np.pi/t_int*np.logspace(-7, 2, 500)
...
def make_los(params):
    ...
    return vec_a, vec_D, D_src

def C_los(omega, Bt, mu_t, Delta2_fun, los):
    vec_a, vec_D, D_src = los
    Ig = I_general(omega, mu_t, vec_D, Delta2_fun)
    integrand = (1 - vec_D/D_src)**2*Ig*(4*np.pi*G_N*rho_m0/vec_a)**2
    return Bt**2*4/omega*np.trapezoid(integrand, vec_D)

# ------------------------------------------------------------------ systems
res = np.load('macro_lens_results.npz')
B_gal = np.max(np.abs(res['eigvals_fit'][1]))     # image B tangential, ~10.2
mu_gal = np.sqrt((B_gal*np.linalg.norm(pB.v_lens_fid)/pB.D_lens)**2 + pB.mu_int_drift**2)
B_clu = np.max(np.abs(pJ.eigvals_fit[2]))         # image C tangential, ~22.6
mu_clu = np.sqrt((B_clu*np.linalg.norm(pJ.v_lens_fid)/pJ.D_lens)**2 + pJ.mu_int_drift**2)
s2_star_pair = 2*1.29e-5*muasyy**2                # galaxy acc: stellar residual (pair)

systems = [('B1422 galaxy, EPIC 0.1 muas', pB, B_gal, mu_gal, 0.1*muas, s2_star_pair),
           ('J1029 cluster, 1 muas', pJ, B_clu, mu_clu, 1.0*muas, 0.0)]

for name, par, Bt, mu_t, sig, s2star in systems:
    los = make_los(par)
    C_noise = sig**2*t_int/N_obs
    s2_acc_noise = 720*sig**2/(t_int**4*N_obs)
    print('\n===== %s  (B = %.1f, mu_tilde = %.2f muas/yr) =====' % (name, Bt, mu_t/muasy))
    for lab, D2f in spectra:
        C_sig = C_los(1.001*omega_min, Bt, mu_t, D2f, los)
        vec_C = np.array([C_los(w, Bt, mu_t, D2f, los) for w in vec_omega])
        s2_acc = np.trapezoid(2*vec_C*vec_omega**4*smear_fac_acc(vec_omega*t_int)/(2*np.pi),
                              vec_omega)
